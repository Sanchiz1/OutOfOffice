...
import { requestProjectEmployees, requestSearchedEmployees } from '../../API/employeeRequests';
import { setGlobalError } from '../../Redux/Reducers/AccountReducer';
import { Employee } from '../../Types/Employee';
import { ShowSuccess } from '../../Helpers/SnackBarHelper';
import { removeFromProject } from '../../API/projectRequests';
...
interface ProjectEmployeesDataTableProps {
    ProjectId: number
}

    export default function ProjectEmployeesDataTable(props: ProjectEmployeesDataTableProps) {
    ...
    React.useEffect(() => {
        requestProjectEmployees(ProjectId, page * rowsPerPage, rowsPerPage, search, orderBy, order).subscribe({
            next(value) {
                setRows(value)
            },
            error(err) {
    ...
    const RemoveUser = (employeeId: number) => {
        removeFromProject(ProjectId, employeeId).subscribe({
            next(value) {
                ShowSuccess(value);
                setPage(1);
            },
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell align="left">
                                            ...
                                            <Button size='small' onClick={() => RemoveUser(row.id)}>Remove</Button>
                                        </TableCell>
                                    </TableRow>
                                );
                            })}
